...
import { ArrowLeft, Clock, CheckCircle, XCircle, CreditCard, User } from "lucide-react-native";

export default function FreelancerPaymentHistoryScreen() {
    ...
    return (
      <View style={styles.card}>
        <View style={styles.cardTop}>
          <View style={[styles.cardIconWrap, isAccepted ? styles.iconAccept : styles.iconReject]}>
            {isAccepted ? <CheckCircle size={22} color="#10b981" /> : <XCircle size={22} color="#ef4444" />}
          </View>
          <View style={styles.cardInfo}>
            <Text style={styles.cardAmount}>${item.price?.toFixed(2)}</Text>
            <Text style={styles.cardFile} numberOfLines={1}>{fileName}</Text>
          </View>
          <View style={[styles.statusBadge, isAccepted ? styles.badgeAccept : styles.badgeReject]}>
            <Text style={[styles.statusText, isAccepted ? styles.textAccept : styles.textReject]}>
        ...
        <View style={styles.clientRow}>
          ...
          <Text style={styles.clientText}>From: <Text style={styles.clientName}>{item.paidBy?.name || "Client"}</Text></Text>
        </View>
    ...
    <View style={styles.container}>
      ...
      {loading ? (
        ...
        <FlatList
          ...
          refreshControl={<RefreshControl refreshing={refreshing} onRefresh={() => { setRefreshing(true); fetchHistory(); }} tintColor="#3b82f6" />}
          ListEmptyComponent={
            <View style={styles.emptyState}>
              <CreditCard size={52} color="#1e293b" />
              <Text style={styles.emptyTitle}>No History Yet</Text>
              <Text style={styles.emptySubtext}>Your accepted and rejected payments will appear here.</Text>
            </View>
          }
        />
      )}
